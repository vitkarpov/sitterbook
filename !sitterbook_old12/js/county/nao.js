$(function(){	
	var rr = Raphael('map12', 700, 750),
		...
		for (var country in nao) {
				
		var obj2 = rr.path(nao[country].path);
				
		obj2.attr(attributes);
		
		arrr[obj2.id] = country;
        
        var newRow = '<div class="parent-check"><p><label><input type="checkbox" name="check" id="'+ obj2.id +'" value="' + nao[country].name + '"/><span></span></label><span class="lab">' + nao[country].name + '</span></p></div>';
            table.append(newRow);
		...
		obj2.click(function(){
		  ...
		  $('#map12').next('.point').remove();
		  $('#map12').after($('<div />').addClass('point'));
		  // удаляем существующий div с классом point и создаём ещё один
		  $('.point')
		  .html('<p>'+nao[arrr[this.id]].name+'</p>')	  
