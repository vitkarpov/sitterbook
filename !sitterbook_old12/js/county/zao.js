$(function(){	
	var rr = Raphael('map17', 700, 600),
		...
		for (var country in zao) {
				
		var obj2 = rr.path(zao[country].path);
				
		obj2.attr(attributes);
		
		arrr[obj2.id] = country;
        
        var newRow = '<div class="parent-check"><p><label><input type="checkbox" name="check" id="'+ obj2.id +'" value="' + zao[country].name + '"/><span></span></label><span class="lab">' + zao[country].name + '</span></p></div>';
            table.append(newRow);
		...
		obj2.click(function(){
		  ...
		  $('#map17').next('.point').remove();
		  $('#map17').after($('<div />').addClass('point'));
		  // удаляем существующий div с классом point и создаём ещё один
		  $('.point')
		  .html('<p>'+zao[arrr[this.id]].name+'</p>')	  
