$(function(){	
	var rr = Raphael('map5', 700, 750),
		...
		for (var country in svao) {
				
		var obj2 = rr.path(svao[country].path);
				
		obj2.attr(attributes);
		
		arrr[obj2.id] = country;
        
        var newRow = '<div class="parent-check"><p><label><input type="checkbox" name="check" id="'+ obj2.id +'" value="' + svao[country].name + '"/><span></span></label><span class="lab">' + svao[country].name + '</span></p></div>';
            table.append(newRow);
		...
		obj2.click(function(){
		  ...
		  $('#map5').next('.point').remove();
		  $('#map5').after($('<div />').addClass('point'));
		  // удаляем существующий div с классом point и создаём ещё один
		  $('.point')
		  .html('<p>'+svao[arrr[this.id]].name+'</p>')	  
