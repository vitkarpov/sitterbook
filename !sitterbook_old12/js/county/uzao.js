$(function(){	
	var rr = Raphael('map19', 700, 800),
		...
		for (var country in uzao) {
				
		var obj2 = rr.path(uzao[country].path);
				
		obj2.attr(attributes);
		
		arrr[obj2.id] = country;
        
        var newRow = '<div class="parent-check"><p><label><input type="checkbox" name="check" id="'+ obj2.id +'" value="' + uzao[country].name + '"/><span></span></label><span class="lab">' + uzao[country].name + '</span></p></div>';
            table.append(newRow);
		...
		obj2.click(function(){
		  ...
		  $('#map19').next('.point').remove();
		  $('#map19').after($('<div />').addClass('point'));
		  // удаляем существующий div с классом point и создаём ещё один
		  $('.point')
		  .html('<p>'+uzao[arrr[this.id]].name+'</p>')	  
