$(function(){	
	var rr = Raphael('map13', 700, 600),
		...
		for (var country in tao) {
				
		var obj2 = rr.path(tao[country].path);
				
		obj2.attr(attributes);
		
		arrr[obj2.id] = country;
        
        var newRow = '<div class="parent-check"><p><label><input type="checkbox" name="check" id="'+ obj2.id +'" value="' + tao[country].name + '"/><span></span></label><span class="lab">' + tao[country].name + '</span></p></div>';
            table.append(newRow);
		...
		obj2.click(function(){
		  ...
		  $('#map13').next('.point').remove();
		  $('#map13').after($('<div />').addClass('point'));
		  // удаляем существующий div с классом point и создаём ещё один
		  $('.point')
		  .html('<p>'+tao[arrr[this.id]].name+'</p>')	  
